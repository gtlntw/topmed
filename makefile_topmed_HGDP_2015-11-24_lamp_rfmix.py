...
######################
#0.2. RFMix: Europe, Africa, Native America subset of HGDP
######################
for chr in xrange(1,23):
	opts["chr"] = chr
...
######################
#0.3. ind. common snp between sample and HGDP subset
######################
opts["id"] = idList[0] # use the first sample to decide the common snps sites between topmed and HGDP
for chr in xrange(1,23):
	opts["chr"] = chr
...
######################
#0.4. LAMPLD create subset reference panel by ancestral populations
######################
...
bcftools index -t -f {outputDir}/temp/{id}/{id}_filtered_chr{chr}.vcf.gz".format(**opts)]
	...
	######################
	opts["param"] = "--mem=20000 --time=0-6" # make srun sh to specify memomry and lower the time needed
...
-o {outputDir}/temp/{id}/{id}_HGDP_chr{chr} --forward-backward --num-threads 1".format(**opts)]
	...
	######################
	#3.1. RFMix subset: merge sample and HGDP subset (filtered HGDP and keep only bi-allelic)
	######################
	for chr in xrange(1,23):
		opts["chr"] = chr
...
bcftools index -t -f {outputDir}/temp/{id}/{id}_HGDP_subset_chr{chr}_filtered_phased.vcf.gz".format(**opts)]
	...
	######################
	#3.2. make files needed for RFMix allele, location, classes and snp position files
	######################
	for chr in xrange(1,23):
		opts["chr"] = chr
...
mv {outputDir}/temp/{id}/{id}_HGDP_subset_chr{chr}.0.Viterbi.txt.tmp {outputDir}/temp/{id}/{id}_HGDP_subset_chr{chr}.0.Viterbi.txt".format(**opts)]
	...
	######################
	#4.1. LAMPLD: convert sample vcf with common snp to HGDP to genotype dosage
	######################
	for chr in xrange(1,23):
		opts["chr"] = chr
